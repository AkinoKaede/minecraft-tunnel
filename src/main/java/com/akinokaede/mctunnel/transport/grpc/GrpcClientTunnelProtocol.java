...
public final class GrpcClientTunnelProtocol implements ClientTunnelProtocol {
	@Override
	public String id() {
		return GrpcTunnelProtocol.ID;
	}
	...
	public Optional<ServerAddress> parseClientAddress(String rawAddress) {
		try {
			...
			if (!"grpc".equalsIgnoreCase(scheme) && !"grpc+h2c".equalsIgnoreCase(scheme)) {
				return Optional.empty();
			}
			...
			if (port < 0 || port > 65535) {
				port = "grpc".equalsIgnoreCase(scheme) ? 443 : 80;
			}
	...
	public void installClient(ChannelPipeline pipeline, ClientTunnelEndpoint endpoint) {
		GrpcClientTunnel.install(pipeline, endpoint);
	}
