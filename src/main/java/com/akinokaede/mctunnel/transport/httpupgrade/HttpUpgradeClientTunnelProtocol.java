...
public final class HttpUpgradeClientTunnelProtocol implements ClientTunnelProtocol {
	@Override
	public String id() {
		return HttpUpgradeTunnelProtocol.ID;
	}
	...
	public Optional<ServerAddress> parseClientAddress(String rawAddress) {
		try {
			...
			if (!"httpupgrade".equalsIgnoreCase(scheme) && !"httpupgrades".equalsIgnoreCase(scheme)) {
				return Optional.empty();
			}
			...
			if (port < 0 || port > 65535) {
				port = "httpupgrades".equalsIgnoreCase(scheme) ? 443 : 80;
			}
			...
			if ("httpupgrades".equalsIgnoreCase(scheme)) {
				tlsSni = host;
	...
	public void installClient(ChannelPipeline pipeline, ClientTunnelEndpoint endpoint) {
		HttpUpgradeClientTunnel.install(pipeline, endpoint);
	}
