...
对于大多数 HTTP 感知型部署，建议优先使用 HTTP Upgrade。它使用支持 WebSocket
的 CDN 和反向代理通常已经支持的 HTTP/1.1 Upgrade 路径，并在升级后直接承载裸
Minecraft 字节，开销比 WebSocket 帧更小。
...
Minecraft 服务端端口接受 `mctunnel.protocol` 中列出的协议。默认启用普通
Minecraft TCP、WebSocket 和 HTTP Upgrade。
...
gRPC 模式是独占的：如果 `mctunnel.protocol` 中出现 `grpc`，服务端会直接安装
gRPC HTTP/2 pipeline，并且不会在该监听器上接受原版 TCP、WebSocket 或 HTTP
Upgrade。
...
| 属性 | 端 | 默认值 | 描述 |
| --- | --- | --- | --- |
| `mctunnel.protocol` | 服务端 | `websocket,httpupgrade,vanilla` | 逗号分隔的已启用服务端协议列表。支持的值：`websocket`、`ws`、`httpupgrade`、`vanilla`、`grpc`。如果出现 `grpc`，它会独占启用并忽略其他值。 |
